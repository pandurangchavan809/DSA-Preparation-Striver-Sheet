...
// 2 Sum Problem | 2 types of the same problem for Interviews.
// Arr[i]+Arr[j]=target return i and j or 2nd problem is return true/false if found or not.
...
public:
    ...
    string twoSumExists(vector<int> arr, int target) {
        ...
        while (left < right) {
            int sum = numsWithIndex[left].first + numsWithIndex[right].first;    // .first is the value at that position as initialized by pair<int, int>
            if (sum == target) {
    ...
    vector<int> twoSumIndices(vector<int> arr, int target) {
        ...
        int left = 0, right = n - 1;
        while (left < right) {
            int sum = numsWithIndex[left].first + numsWithIndex[right].first;     // .first is the value at that position as initialized by pairs
            if (sum == target) {
                // Return original indices
                return {numsWithIndex[left].second, numsWithIndex[right].second};   // .second is the original index at that position
            } else if (sum < target) {
                left++;
            } else {
                right--;
